...
async function testSagaFlow() {
  ...
  try {
    ...
    console.log(`✅ 아이템 생성: ${item.name} (가격: ${item.price}원, 재고: ${item.stock}개)\n`);
    ...
    if (finalOrder.status === 'COMPLETED' && userBalance === 10000 && userInventory.length === 1) {
      console.log('✅ 테스트 1 성공: Saga 플로우가 정상적으로 완료되었습니다!\n');
    } else {
    ...
    if (failedOrder.status === 'FAILED' && finalBalance === 10000) {
      console.log('✅ 테스트 2 성공: 실패 시나리오가 정상적으로 처리되었습니다!\n');
    } else {
      console.log('❌ 테스트 2 실패: 보상 트랜잭션이 제대로 동작하지 않았습니다.\n');
    }
